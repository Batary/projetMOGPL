...
""" Réponse à la question Q4:
 ...
 @return : True si possible de colorier toute la ligne avec tous les blocs, False sinon
"""
	...
	for l in range(2, len(line_sequence) + 1):  # cas où ou considère au moins 2 blocs
		for j in range(line_length):
			if j < sum(line_sequence[
				...
				# - soit le dernier bloc finit sur la case j+1. Dans un tel cas, T[j][l] est vrai ssi on peut faire rentrer les blocs précédents dans les cases
				#	 devant le début de ce dernier bloc (il ne faut pas oublier l'espace blanc entre deux blocs!)
				
				# - soit la dernière case est blanche. Dans ce cas, T[j][l] ssi T[j-1][l]
				T[j][l] = T[j - line_sequence[l - 1] - 1][l - 1] or T[j - 1][l]
	
	# On renvoit la "dernière" case du tableau indiquant si on peut colorier toute la ligne avec toute la séquence
	return T[-1][-1]
...
def colour_line_bis(line, line_sequence):
	# si la séquence est vide (aucun bloc), la réponse est TRUE ssi il n'y a pas de case déjà coloriée en noir.
	if not line_sequence:
		return (const.BLACK not in line)
	
	# si la ligne est entièrement colorée, il suffit de vérifier si le nombre de colonnes noires est égal à la somme des longueurs des blocs
	...
	# peut etre noire ou blanche, sachant que les autres cases sont colorées de la bonne manière	(UNIQUE); grace à cette unicité, la réponse est OUI
	...
	for j in range(line_length):
		# on peut colorier les j+1 premieres cases avec 0 blocs ssi il n'y a pas de case noire (qui imposerait la présence d'un bloc)
	...
	for l in range(len(line_sequence)):  # l correspond à (l+1) blocs considérés
		for j in range(0, sum(line_sequence[0:l + 1]) + l - 1):
			# si j est inferieur a la somme des longueurs des (l+1) blocs considérés + l (si l+1 blocs, au moins l cases de plus pour les séparer),
			# les blocs ne rentrent pas - T[j][l+1] est fausse
			T[j][l + 1] = False
	
	# Cas 2b
	# si on considère un seul bloc: T[j][1] = 1 si j+1 est la longueur du bloc et qu'il n'y a pas de cases blanches
	j = line_sequence[0] - 1
	...
	for l in range(len(line_sequence)):  # pour chacun des blocs l correspond aux l+1 blocs considérés
		for j in range(sum(line_sequence[0:l + 1]) + l - 1,
			...
			# 1) La derniere case ne peut pas etre blanche, ainsi que les cases precedentes qui forment le bloc:
			last_black = const.WHITE not in (line[j - line_sequence[l] + 1:j + 1])
			# 2) La case avant n'existe pas ou n'est pas coloriée en noir.
			box_before = (j - line_sequence[l] < 0 or line[j - line_sequence[l]] != const.BLACK)
			# 3) La case d'après n'existe pas ou n'est pas coloriée en noir.
			box_after = (line_length == j + 1 or line[j + 1] != const.BLACK)
			# 4) La relation de récursion est bien vérifiée - on peut remplir les cases avant le début de ce bloc (et avant la case "de séparation")
			...
			#	- la somme des cases noires après la case j+1 (on finit sur j, il faut pas oublier la séparation) ne dépasse pas la somme des longueurs des blocs suivants
			#	- la somme des cases noires avant le début de ce dernier bloc ne dépasse pas la somme des longueurs des blocs précédents
			...
			# de plus, on calcule les cases noires avant/après de façon analogue qu'avant
			...
			# ENFIN, T[j][l+1] vrai si au moins une des deux confgurations ci-dessus est possible
